...
// Create magazine
router.get("/coordinator-create-magazine", async (req, res) => {res.render("coordinator/coordinator-create-magazine");});
router.post("/coordinator-create-magazine", async (req, res) => {
    const connection = await pool.getConnection();
    const { title, content, file } = req.body;
    const currentDate = new Date().toISOString().slice(0, 19).replace('T', ' '); // Get current date and time in the format YYYY-MM-DD HH:MM:SS
    const [rows] = await connection.query(
        "INSERT INTO magazine(magazine_title, magazine_content, magazine_file, magazine_created_at, magazine_updated_at) VALUES (?, ?, ?, ?, ?)",
        [title, content, file, currentDate, currentDate]
    );
    connection.release();
    res.redirect("/coordinator/coordinator-page");
});


// Manage magazine
router.get("/coordinator-manage-magazine", async (req, res) => {
    const connection = await pool.getConnection();
    const [rows] = await connection.query("SELECT * FROM magazine");
    res.render("coordinator/coordinator-manage-magazine", { title: "Manage Magazine", magazines: rows });
});


// Edit magazine
router.get("/coordinator-edit-magazine/:magazineId", async (req, res) => {
    const connection = await pool.getConnection();
    const [rows] = await connection.query(
    ...
    connection.release();
    res.render("coordinator/coordinator-edit-magazine", {
        title: "Update Magazine",
        magazine: rows[0],
    });
});
router.post("/coordinator-edit-magazine/:magazineId", async (req, res) => {
    const connection = await pool.getConnection();
    const { title, content, file } = req.body;
    const currentDate = new Date().toISOString().slice(0, 19).replace('T', ' '); // Get current date and time in the format YYYY-MM-DD HH:MM:SS
    const [rows] = await connection.query(
        "UPDATE magazine SET magazine_title = ?, magazine_content = ?, magazine_file = ?, magazine_updated_at = ? WHERE magazine_id = ?",
        [title, content, file, currentDate, req.params.magazineId]
    );
    connection.release();
    res.redirect("/coordinator/coordinator-manage-magazine");
});
